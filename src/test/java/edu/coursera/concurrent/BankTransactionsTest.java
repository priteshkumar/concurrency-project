...
public class BankTransactionsTest extends TestCase {
  ...
  private static void testForDeadlock(final ThreadSafeBankTransaction impl){
    ...
    finish(() -> {
      for (int i = 0; i < 10000; i++) {
        ...
        async(() -> {
          ...
          int srcIndex = ii % 2 == 0?0:1;//randomIntValue(myRandom,2);
          ...
          final int transferAmount = randomIntValue(myRandom,
              srcAccount.balance());
          //System.out.println("issue transfer " + srcIndex + " " + destIndex);
  ...
  public void testObjectIsolation() {
    // warmup
    //testForDeadlock(new BankTransactionUsingLockOrdering());
    ...
    testDriver(new BankTransactionUsingLockOrdering());
    //final long objectTime = testDriver(
    //    new BankTransactionsUsingObjectIsolation());
    final long objectTime = testDriver(
        new BankTransactionUsingLockOrdering());
